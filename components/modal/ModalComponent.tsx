...
const MessageBubble = ({ msg, isLast }: { msg: Message, isLast: boolean }) => {
    const { displayedText, isComplete } = useTypewriter(msg.content, isLast && msg.role === 'assistant' ? 20 : 0);

    // Use displayedText only if it's the last message and from assistant, otherwise show full content immediately (for history)
    const textToShow = (isLast && msg.role === 'assistant') ? displayedText : msg.content;
                            ...
                            <div className="relative z-10 flex-1 overflow-y-auto p-5 scrollbar-hide space-y-4 pb-36 pt-8">
                                {messages.length === 0 && (
                                    <div className="flex flex-col items-center justify-center h-[60%] text-center text-white/80 p-6">
                                        ...
                                        <p className="font-bold text-lg text-[#C6A664]">Halo! Saya asisten Warisan Nusantara.</p>
                                        <p className="mt-2 text-sm text-white/70 leading-relaxed">Tanyakan apa saja tentang tempat wisata, sejarah, atau budaya Indonesia!</p>
                                    </div>
                                )}

...
export async function POST(req: Request) {
    try {
        ...
        const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
            ...
            body: JSON.stringify({
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "Kamu adalah asisten virtual yang HANYA menjawab pertanyaan seputar tempat wisata, budaya, dan sejarah di Indonesia. Jika pengguna bertanya tentang hal lain (seperti coding, politik, matematika umum yang tidak relevan), tolah dengan sopan dan katakan bahwa kamu hanya bisa membahas warisan nusantara. Jawab dengan ringkas, ramah, dan informatif."
                    },
                    {
                        "role": "user",
                        "content": message
        ...
        if (!response.ok) {
            return NextResponse.json({ error: `OpenRouter API Error: ${response.status} - ${responseText}` }, { status: response.status });
        }
        ...
        try {
            ...
            return NextResponse.json({ error: "Invalid JSON response from OpenRouter" }, { status: 500 });
        }
        ...
        if (data.error) {
            ...
            return NextResponse.json({ error: data.error.message || "Error from AI provider" }, { status: 500 });
        }
